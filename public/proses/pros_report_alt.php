...
//nama file dan judul report
if ($from != '') {
    $fromz = $from;
} else {
    $sql2 = mysqli_query($con, "SELECT MIN(DISTINCT DATE(start_date)) AS fromm FROM t_transaksi");
    $data2 = mysqli_fetch_array($sql2);
    $fromz = $data2['fromm'];//tanggal paling awal
}
...
if ($div != '0') {
    $sql3 = mysqli_query($con, "SELECT Nama_div FROM t_divisi WHERE kode_div='$div'");
...
if ($cat != '0') {
    $sql4 = mysqli_query($con, "SELECT category_name FROM t_category WHERE id='$cat'");
    $data4 = mysqli_fetch_array($sql4);
    $catz = $data4['category_name'];
} else {
    $catz='All';
}

if ($catid != '0') {
    $sql5 = mysqli_query($con, "SELECT detail_category_name FROM t_category_detail WHERE id='$catid'");
    $data5 = mysqli_fetch_array($sql5);
    $catidz = $data5['detail_category_name'];
} else {
    $catidz='All';
}

if ($flag != '') {
    $sql6 = mysqli_query($con,
"SELECT DISTINCT
CASE 
...
$excel->setActiveSheetIndex(0)->setCellValue('A1', "Report Ticket divisi $divz kategori $catz ($catidz) $fromz s/d $toz status $flagz"); // Set kolom A1 dengan tulisan "DATA SISWA"
...
$excel->setActiveSheetIndex(0)->setCellValue('K3', "PROBLEM"); // Set kolom F3 dengan tulisan "ALAMAT"
$excel->setActiveSheetIndex(0)->setCellValue('L3', "PENYELESAIAN"); // Set kolom F3 dengan tulisan "ALAMAT"
$excel->setActiveSheetIndex(0)->setCellValue('M3', "WAKTU MULAI"); // Set kolom F3 dengan tulisan "ALAMAT"
$excel->setActiveSheetIndex(0)->setCellValue('N3', "WAKTU SELESAI"); // Set kolom F3 dengan tulisan "ALAMAT"
$excel->setActiveSheetIndex(0)->setCellValue('O3', "WAKTU HOLD (JAM)"); // Set kolom F3 dengan tulisan "ALAMAT"
$excel->setActiveSheetIndex(0)->setCellValue('P3', "WAKTU PENGERJAAN (JAM)"); // Set kolom F3 dengan tulisan "ALAMAT"
$excel->setActiveSheetIndex(0)->setCellValue('Q3', "PRIORITAS"); // Set kolom F3 dengan tulisan "ALAMAT"
$excel->setActiveSheetIndex(0)->setCellValue('R3', "STATUS TIKET"); // Set kolom F3 dengan tulisan "ALAMAT"
...
if ($from != '') {
    ...
    $sql1 = mysqli_query($con, "SELECT MIN(DISTINCT DATE(start_date)) AS fromm FROM t_transaksi");
    $data1 = mysqli_fetch_array($sql1);
    $froms = $data1['fromm'];//tanggal paling awal
}
...
//div
if ($div != '0') {
    $divs = "SELECT DISTINCT '$div' FROM t_divisi WHERE flag='1'";
} else {
    $divs = "SELECT kode_div FROM t_divisi WHERE flag='1'";//all divisi
}

//cat
if ($cat != '0') {
    $cats = "SELECT DISTINCT '$cat' FROM t_category WHERE flag='1'";
} else {
    $cats = "SELECT id FROM t_category WHERE flag='1'";//all divisi
}

//catid
if ($catid != '0') {
    $catids = "SELECT DISTINCT '$catid' FROM t_category_detail WHERE flag='1'";
} else {
    $catids = "SELECT id FROM t_category_detail WHERE flag='1'";//all divisi
}
...
WHERE a.id>1 AND i.status_pms='1' AND i.flag_input='1'
AND DATE(a.start_date) BETWEEN '$froms' AND '$tos'
AND a.id_divisi IN ($divs)
AND e.id IN ($cats)
AND d.id IN ($catids)
AND a.flag IN ($flags)
ORDER BY a.no_ticket ASC";
...
while($data = mysqli_fetch_array($sql)){ // Ambil semua data dari hasil eksekusi $sql
	...
	$excel->setActiveSheetIndex(0)->setCellValue('K'.$numrow, $data['problem']);
	$excel->setActiveSheetIndex(0)->setCellValue('L'.$numrow, $data['penyelesaian']);
	$excel->setActiveSheetIndex(0)->setCellValue('M'.$numrow, $data['start_date']);
	$excel->setActiveSheetIndex(0)->setCellValue('N'.$numrow, $data['finish_date']);
	$excel->setActiveSheetIndex(0)->setCellValue('O'.$numrow, $data['Hold_Time']);
	$excel->setActiveSheetIndex(0)->setCellValue('P'.$numrow, $data['Work_Time']);
	$excel->setActiveSheetIndex(0)->setCellValue('Q'.$numrow, $data['priority']);
